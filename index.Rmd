...
Dernière version : Novembre 2019.
...
### Quelques statistiques descriptives sur les étapes de détection des doublons

Pour obtenir quelques informations sur le nombre d'arbres d'utilisateurs par département, et sur le nombre d'individus que contiennent leurs arbres, nous proposons quelques statistiques descriptives. Le Tableau ci-dessous fournit le nombre d'individus dans les arbres des utilisateurs, dans chacun des découpages de la base effectués par département. On lit ainsi que pour le département 01 (l'Ain), le nombre d'individus présents dans les 3916 arbres des utilisateurs ayant reporté au moins un ancêtre né dans l'Ain entre 1800 et 1804, s'élève à 186 160. À l'issue de la première étape de simplification, ce nombre d'individus tombe à 139 168. Ainsi, la première étape de simplification a identifié 46 992 doublons (c.f. colonne `nb_doublons_1`). À l'issue de la sixième simplification, le nombre d'individus présents dans les arbres pour le tronçon de données correspondant à l'ain s'élève à 123 412. Aussi, le nombre de doublons identifiés lors du passage des six algorithmes s'élève à 62 748 (c.f. colonne `nb_doublons_fin`).
...
Comme l'indique la colonne intitulée `pct_etape_1`, le pourcentage de doublons repérés lors de la première simplification représente $74,89\%$ de l'ensemble des doublons reprérés pour le tronçon de l'Ain. On peut noter que pour l'ensemble des départements, ce pourcentage demeure élevé, allant de $63,99\%$ (06 Alpes-Maritimes) à $86,34\%$ (88 Vosges), la moyenne s'établissant à $80,37\%$. La Figure ci-dessous indique le nombre d'individus non détectés comme étant des doublons à chacune des étapes de simplification, pour chaque département (les points d'un département sont reliés).
...
ggplot(data = nb_obs_depts %>%
  ...
  geom_line(alpha = .2) +
  geom_point(alpha=.2) +
  scale_x_continuous(breaks = 0:5, labels = c("Données brutes", "Etape 1", "Etape 2", "Etape 3", "Etape 4", "Etapes 5 et 6")) +
  xlab("Étape") + ylab("Nombre d'individus (x 1000)")
```


Il peut être utile de donner quelques exemples de doublons identifiés. Prenons un exemple spécifique de doublons détecté dans le département 35, Ille-et-Vilaine. L'application des différents algorithmes a permis de désigner que parmi les 368 340 individus (présents dans 7 474 arbres distincts), 7 d'entre-eux sont susceptibles de faire mention de la même personne, à savoir un certain Jean Esnault né dans la commune nommée Montreuil-sous-Pérouse, le 15 août 1837 (cf Tableau ci-dessous). On note que dans cet exemple, le nom, le prénom et la date de naissance est commune à toutes les observations. Le regroupement de ces observations en une seule permet :

- d'attribuer une date de décès plus précise pour les observations ne mentionnant ni le mois ni l'année (les valeurs indiquent 00 et 00) ;
- de corriger la date de décès de l'observation 2, qui est observée moins fréquemment que le 15 juin 1858 (on note toutefois qu'il est possible que la vraie date de décès soit celle indiquée à l'observation 2, nous avons émis l'hypothèse qu'en cas d'informations différentes, celle la plus fréquemment observée serait retenue) ;
- de compléter les branches manquantes pour le père dans toutes les observations en prenant la valeur donnée pour la deuxième comme référence.


|nom     |prenom |lieu_N                        |dept_N |date_N   |lieu_D                        |dept_D |date_D   |prenom_mere |prenom_pere |
|:-------|:------|:-----------------------------|:------|:--------|:-----------------------------|:------|:--------|:-----------|:-----------|
...
Prenons un deuxième exemple, celui d'une certaine Clarisse Dupuy-Fromy, née à Saint-Malo le 20 septembre 1803. Dans les données concernant l'Ille-et-Vilaine, 6 utilisateurs de Généanet ont mentionné cette personne, avec plus ou moins de détails. La mise en commun des arbres de ces 6 personnes permet de définir une date de naissance, une de décès et un lien avec les deux parents. On note dans le tableau ci-dessous que le jour de naissance ne fait pas l'objet d'un accord entre les différents généalogistes. Notre algorithme désigne la valeur finale en fonction de la fréquence d'observation ; ainsi, dans les données finales, il est supposé que le jour de naissance de Clarisse Dupuy-Fromy est le 20 septembre 1803. La date de décès ne pourras pas bénéficier d'une précision accrue grâce à une des observations, seule l'année est mentionnée, 1833.
...
Nous allons construire cet tableau génération par génération. Nous définissons à cet effet un tableau contenant la génération courante, qui sera modifié quand on s'intéressera aux enfants, petits-enfants, etc. Il s'agit de l'initialiser.
...
Identifions à présent les enfants de la génération courante.
...
# Examen de la mortalité des français au début du XIXe siècle

Dans cette section, nous présentons les codes développés pour étudier la mortalité à partir des données de généanlogie collaborative. Nous nous concentrons principalement sur les aïeux de la base, à savoir les individus nés entre 1800 et 1804.
...
Nous avons également besoin de récupérer des données de mortalité. Nous téléchargeons les tables de mortalités de [Vallin *et al.* (2001)](http://www.lifetable.de/cgi-bin/country.php?code=fra). Plus particulièrement, nous prenons appui sur les tables de mortalité de générations (tableaux III-A-1 et III-A-2).

```{r, eval=FALSE}
# Tables de mortalité
# Tables de mortalité donnant, pour chacune des générations 1702 à 1805,
# les survivants, les quotients, les décès, les espérances de vie et les 
# populations stationnaires à compter du premier âge atteint dans l'année 1806 
# (tables démarrant à un âge supérieur à 0 an). Sexe masculin / Sexe féminin.
# https://www.ined.fr/Xtradocs/cdrom_vallin_mesle/Tables-de-mortalite/Tables-de-generation/Tables-gen-incompl.htm
load_table_vallin_mesle <- 
  function(year, sex){
    gender <- ifelse(sex == "fem", yes = "2", no = "1")
    file_name <- str_c("mortalite/", sex, "_", year, ".txt")
    x <- read_fwf(file_name, skip = 5,
                  col_positions = fwf_widths(c(4, 8, 8, 8, 6, 8), c("x", "Sx", "D(x,x+a)", "aQx", "Ex", "P(x,x+a)"))) %>% 
      mutate(cohorte = year, sex = gender)
    x
  }

tables_mortalite <- NULL
for(i in 1800:1804){
  tables_mortalite <- 
    tables_mortalite %>% 
    bind_rows(
      load_table_vallin_mesle(i, "fem") %>% 
        bind_rows(load_table_vallin_mesle(i, "masc"))
    )
}
```

Nous calculons, pour une cohorte donnée, la probabilité de décès instantané $\mu_x$ (qui est équivalente à $q_x$, c'est à dire la probabilité qu'un individu âgé de $x$ ne survive pas en $x+1$). Nous calculons également la probabilité qu'un individu âgé de $x$ atteigne $x+1$, soit $p_x$, et la probabilité de survie conditionnelle à l'âge $S_x(t)$.

```{r, eval=FALSE}
survies_diagonale_1800_1804_new <- 
  tables_mortalite %>%
  group_by(cohorte, sex) %>% 
  mutate(px = 1-aQx) %>% 
  mutate(sx = cumprod(px)) %>% 
  mutate(mx = `D(x,x+a)`/Sx) %>% 
  ungroup() %>% 
  mutate(sex = as.character(sex)) %>% 
  rename(age = x, sexe = sex)
...
```{r, echo=FALSE}
load("../git/rda_files/survies_diagonale_1800_1804_new.rda")
```

```{r, eval=TRUE, echo=FALSE}
head(survies_diagonale_1800_1804_new)
```
...
Les estimations sont données par cohorte, nous les agrégeons pour le groupe 1800-1804, en prenant la moyenne des probabilités de survie et de décès instantané, par âge et sexe.
...
surv_geneanet_tot <- surv_geneanet_c
```
...
### Estimations à partir des données de généalogie, pour l'ensemble des individus
...
```{r, echo=F}
load("../git/rda_files/surv_geneanet_2_tot_c.rda")
```


### Estimations à partir des données de généalogie, par département
...
## Estimation de la force de mortalité
...
### Pour les données de Vallin & Meslé comparées à celles de Geneanet
...
### Pour l'ensemble des individus de Geneanet, par département
...
À l'aide des estimations des fonctions de survie pour les femmes et les hommes, il est aisé de calculer les espérances de vie résiduelle des individus.

### Pour Vallin et Meslé (2010)

Pour commencer, nous pouvons calculer l'espérance de vie à la naissance, par sexe :
...
Calculons l'espérance de vie résiduelle avec les données de Vallin et Meslé.

```{r}
esperance_vie_vallin <- 
  survies_diagonale_1800_1804_new %>% 
  group_by(cohorte, sexe) %>% 
  mutate(sum_s = sum(Sx) - cumsum(Sx)) %>% 
  mutate(ex = .5+(sum_s)/Sx)
```


### Pour l'ensemble des aïeux

À partir des tables de mortalité obtenues avec les données de généalogie :

```{r, eval=TRUE}
esperance_vie_geneanet <- 
  surv_geneanet_2_tot_c %>% 
  group_by(annee_naissance) %>% 
  mutate(sum_expo_f = sum(exposition_f) - cumsum(exposition_f),
         sum_expo_h = sum(exposition_h) - cumsum(exposition_h)) %>% 
  mutate(ex_f = .5+(sum_expo_f)/exposition_f,
         ex_h = .5+(sum_expo_h)/exposition_h)

esperance_vie_geneanet
```

On peut ensuite préparer un tableau contenant pour chaque âge et chaque sexe de chaque cohorte, l'espérance de vie obtenue à partir des tables de mortalité du moment de Vallin et Meslé et celle obtenue avec les données de généalogie :

```{r, eval=TRUE}
esperance_vie_residuelle <- 
  esperance_vie_vallin %>% 
  select(age, cohorte, sexe, ex) %>% 
  full_join(
    esperance_vie_geneanet %>% 
      select(annee_naissance, age, ex_f, ex_h) %>% 
      rename(cohorte = annee_naissance) %>% 
      gather(sexe, ex_geneanet, ex_f, ex_h) %>% 
      mutate(sexe = ifelse(sexe == "ex_f", yes = "2", no = "1"))
  ) %>% 
  arrange(cohorte, age)
esperance_vie_residuelle
```

Quelques manipulations pour l'esthétisme des résultats :

```{r}
esperance_vie_residuelle <- 
  esperance_vie_residuelle %>% 
  ungroup() %>% 
  gather(type, residual_ex, ex, ex_geneanet) %>% 
  mutate(sexe = factor(sexe, levels = c("2", "1"), labels = c("Femmes", "Hommes"))) %>% 
  mutate(type = factor(type, levels = c("ex_geneanet", "ex"), labels = c("Geneanet", "Tables de mortalité du moment de Vallin et Meslé (2001)")))
...
```{r}
p <- 
  ggplot(data = esperance_vie_residuelle, aes(x = age, y = residual_ex)) +
  geom_line(aes(colour = sexe, linetype = type)) +
  facet_wrap(~cohorte) +
  labs(x = "Âge", y = "Espérance de vie résiduelle",
       title = "Espérance de vie résiduelle par cohorte et par sexe\n estimées à partir de Geneanet\net avec les tables de mortalité de Vallin et Meslé") +
  scale_colour_manual("", values = c("Femmes" = "red", "Hommes" = "blue")) +
  scale_linetype_manual("", values = c("Tables de mortalité du moment de Vallin et Meslé (2001)" = "dashed",
                                       "Geneanet" = "solid")) +
...
## Intervalles de confiance pour les espérances de vie à la naissance

Nous allons estimer des intervalles de confiance pour les espérances de vie à la naissance. Pour ce faire, nous suivons une méthode s'appuyant sur du *bootstrap* proposée Carlo Giovanni Camarda sur son site (https://sites.google.com/site/carlogiovannicamarda/r-stuff/life-expectancy-confidence-interval).
...
### Pour Vallin et Meslé

Définissons une fonction spécifiquement pour les données de Vallin et Meslé.
...
ex_ci_vallin <- function(LT, ns=10000, which.x=0, level=0.95){
  
  LT <- LT %>% filter(age < 105)
  
  ## cohort
  cohort <- LT$cohorte %>% unique()
  ## sex
  sex <- LT$sexe %>% unique()
  ## ages
  x <- LT$age
  ...
  ## for all replicates, compute life expectancy
  fun.ex <- function(qx){
    lifetable.qx(x=x, qx, sex=sex)$ex[wh]
  }
  exsim <- apply(QX, 2, fun.ex)
  
  ## confidence interval
  CI <- quantile(exsim, na.rm = T,
                 probs = c((1-level)/2,
                           1 - (1-level)/2))
  
  data.frame(ex_mean = mean(exsim, na.rm=T),
             ex_inf = CI[[1]],
             ex_sup = CI[[2]],
             age = which.x,
             sexe = sex,
...
Les tables de mortalité par sexe :

```{r, eval=FALSE}
LT_h <- 
  survies_diagonale_1800_1804_new %>%
  filter(cohorte == cohort, sexe == 1)
LT_f <-
  survies_diagonale_1800_1804_new %>%
  filter(cohorte == cohort, sexe == 2)
```

Pour chaque âge, nous pouvons appliquer la fonction `ex_ci_vallin()`, aux deux tables :

```{r, eval=FALSE}
ex_vallin_cohort <-
  pblapply(LT_h$age, function(age){
    ex_ci_vallin(LT = LT_h, which.x=age) %>%
      bind_rows(ex_ci_vallin(LT = LT_f, which.x=age))
  }) %>%
  bind_rows()

ex_vallin <-
  ex_vallin %>%
  bind_rows(ex_vallin_cohort)
```

```{r, echo=FALSE}
load("../git/rda_files/ex_vallin.rda")
```
...
Nous proposons une fonction, `ex_ci_cohorte()`, qui à partir d'une année de naissance, estime par bootstrap les intervalles de confiance pour l'espérance de vie à la naissance de la cohorte d'individus nés cette année.
...
#'@param ns number of samples to draw
#'@param level level for the confidence interval
ex_ci_cohorte <- function(LT, sex, ns=10000, level=0.95){
  
  LT <- LT %>% filter(age < 105)
  
  ## cohort
  cohort <- LT$annee_naissance %>% unique()
  ## ages
  x <- LT$age
  ...
  ## for all replicates, compute life expectancy
  fun.ex <- function(qx){
    lifetable.qx(x=x, qx, sex=sex)$ex
  }
  
  exsim <- apply(QX, 2, fun.ex)
  apply(exsim, 1, function(x){
    CI <- quantile(x, na.rm = T,
                   probs = c((1-level)/2,
                             1 - (1-level)/2))
    tibble(ex_mean = mean(x, na.rm=T),
           ex_inf = CI[[1]],
           ex_sup = CI[[2]],
           sexe = sex,
           cohort = cohort)
...
}# End of ex_ci_cohorte()
```

```{r, eval=FALSE}
ex_geneanet <- 
  pblapply(1800:1804, function(cohort){
  LT <- surv_geneanet_2_tot_c %>% filter(annee_naissance == cohort)
  ex_f_tmp <- ex_ci_cohorte(LT = LT, sex = 2, ns = 10000, level = .95)
  ex_h_tmp <- ex_ci_cohorte(LT = LT, sex = 1, ns = 10000, level = .95)
  ex_f_tmp %>% 
    bind_rows(ex_h_tmp)
  })
...
```{r, echo=FALSE}
load("../git/rda_files/ex_geneanet.rda")
```
...
Il est possible de représenter graphiquement l'espérance de vie résiduelle en fonction de l'âge, pour chaque cohorte, en différentiant par genre (pour les estimations réalisées avec les tables de mortalité du moment de Vallin et Meslé, ainsi que pour celles réalisées avec les données de généalogie).
...
df_plot_ex <- 
  ...
  mutate(type = factor(type, levels = c("Geneanet", "Historique"), labels = c("Geneanet", "Tables de mortalité du moment de Vallin et Meslé (2001)"))) %>% 
  tbl_df()
```

```{r}
p <- ggplot(data = df_plot_ex, aes(x = age, y = ex_mean, colour = sexe, linetype = type)) +
  geom_ribbon(aes(ymin = ex_inf, ymax = ex_sup, fill = sexe), alpha = .4) +
  geom_line() +
  facet_grid(sexe~cohort) +
  labs(x = "Âge", y = "Espérance de vie résiduelle",
      title = "Espérance de vie résiduelle par cohorte et par sexe\n estimées à partir de Geneanet\net avec les tables de mortalité de Vallin et Meslé") +
  scale_colour_manual("", values = c("Femmes" = "red", "Hommes" = "blue")) +
...
Il est possible de visualiser la différence entre la valeur estimée à partir des données de généalogie et celle estimée à partir des données de Vallin et Meslé ($E_x^{\text{Geneanet}} - E_x^{\text{Vallin \& Meslé}}$) :
...
df_tmp <- 
  ...
  tbl_df() %>% 
  select(ex_mean, type, age, sexe, cohort) %>% 
  spread(type, ex_mean) %>% 
  mutate(difference = Geneanet - Historique)
```

```{r}
ggplot(data = df_tmp, aes(x = age, y = difference, colour = sexe)) +
  geom_line() +
  facet_wrap(~cohort) +
  scale_colour_manual("", values = c("Femmes" = "red", "Hommes" = "blue")) +
  labs(x = "Âge", y = "Différence",
       title = "Différence des estimations d'espérance de vie résiduelle\ncalculées avec les données de Geneanet\net à partir des tables de mortalité du moment de Vallin et Meslé.") +
  theme(legend.position = "bottom")
```

La moyenne de ces écarts pour chaque cohorte :

```{r}
df_tmp %>% 
  group_by(cohort, sexe) %>% 
  summarise(mean(difference, na.rm=T))
```

La moyenne des écarts, toutes cohortes confondues :
...
### Pour les aïeux, par département
...
```{r, eval=FALSE}
# Esperance de vie à la naissance uniquement
...
#'@param ns number of samples to draw
#'@param level level for the confidence interval
ex_ci_cohorte_dep <- function(LT, dept_birth, sex, ns=10000, level=0.95){
  
  LT <- LT %>% filter(age < 105) %>% 
    filter(dept == dept_birth)
  ...
  ## for all replicates, compute life expectancy
  fun.ex <- function(qx){
    lifetable.qx(x=x, qx, sex=sex) %>% 
      filter(x == 0) %>% 
      magrittr::extract2("ex")
  }
  
  exsim <- apply(QX, 2, fun.ex)
  
  ex_mean <- mean(exsim)
  ex_inf <- quantile(exsim,(1-level)/2)
  ex_sup <- quantile(exsim,1 - (1-level)/2)
  
  tibble(ex_mean = ex_mean,
             ex_inf = ex_inf,
             ex_sup = ex_sup,
             sexe = sex,
             annee_naissance = cohort,
             dept = dept_birth)
  
}# End of ex_ci_cohorte_dep()
```



Nous bouclons sur les années de naissance des aïeux, pour chaque sexe :

```{r, eval=FALSE}
depts <- sort(unique(surv_geneanet_2_depts_c$dept))
ex_depts_geneanet_tmp <- 
    pblapply(depts, function(dept){
      LT <- surv_geneanet_2_depts_c %>% filter(annee_naissance == cohort)
      ex_ci_cohorte_dep(LT = LT, dept_birth = dept, sex = 2, ns = 10000, level = .95) %>%
        bind_rows(
          ex_ci_cohorte_dep(LT = LT, dept_birth = dept, sex = 1, ns = 10000, level = .95)
        )
    })
```

```{r, echo=FALSE}
load("../git/rda_files/ex_depts_geneanet.rda")
```


```{r}
head(ex_depts_geneanet)
```


#### Ensemble des individus nés entre 1800 et 1804

Nous calculons les espérances de vie résiduelle par département, en considérant les individus nés entre 1800 et 1804 comme une seule cohorte.

```{r}
surv_geneanet_2_depts_c_2 <- 
  surv_geneanet_2_depts_c %>%
  select(age, exposition_f, exposition_h, deces_f, deces_h, dept) %>% 
  group_by(age, dept) %>% 
  summarise(exposition_f = sum(exposition_f),
            exposition_h = sum(exposition_h),
            deces_f = sum(deces_f),
            deces_h = sum(deces_h)
            ) %>% 
  ungroup() %>% 
...
On peut ainsi, comme précédemment, calculer la survie :
...
Reste alors à calculer les intervalles de confiance, par bootstrap :

```{r, eval=FALSE}
depts <- sort(unique(surv_geneanet_2_depts_c_2$dept))
ex_depts_geneanet_2 <-
  pblapply(depts, function(dept){
    LT <- surv_geneanet_2_depts_c_2
    LT <- LT %>% mutate(annee_naissance = "1800-1804")
    ex_ci_cohorte_dep(LT = LT, dept_birth = dept, sex = 2, ns = 10000, level = .95) %>%
      bind_rows(
        ex_ci_cohorte_dep(LT = LT, dept_birth = dept, sex = 1, ns = 10000, level = .95)
      )
...
```{r, echo=FALSE}
load("../git/rda_files/ex_depts_geneanet_2.rda")
```
...
```{r}
esperance_vie_naissance_map <- 
  france_1800 %>% 
  left_join(
    ex_depts_geneanet_2 %>% 
      select(dept, ex_mean, sexe), by = c("departement" = "dept")) %>% 
  mutate(sexe = factor(sexe, levels = c(2, 1), labels = c("Femmes", "Hommes")))

p_map <- 
  ggplot(esperance_vie_naissance_map,
         aes(x = long, y = lat, group = group, fill = ex_mean)) +
  geom_polygon() +
  facet_grid(~sexe) +
...
Il est intéressant de comparer les valeurs estimées d'espérance de vie à la naissance avec celles qui existent dans la littérature. Nous reportons les valeurs trouvées dans @van_de_Walle_1973 (valeurs pour les femmes uniquement).
...
```{r}
ex_depts_comparison_depts <- 
  ex_depts_geneanet_2 %>% 
  filter(sexe == 2) %>% 
  left_join(esperances_vandewalle)

head(ex_depts_comparison_depts)
```

L'espérance de vie moyenne à la naissance des femmes vaut, pour notre échantillon et pour celui de van de Walle :

```{r}
mean(ex_depts_comparison_depts$ex_mean, na.rm=T)
mean(ex_depts_comparison_depts$e_f_walle, na.rm=T)
```

La corrélation s'élève quant à elle à :
```{r}
cor(ex_depts_comparison_depts$ex_mean, ex_depts_comparison_depts$e_f_walle, use = "pairwise.complete.obs")
```
...
ex_depts_comparison_depts <- 
  ex_depts_comparison_depts %>% 
  mutate(e_f_3 = ex_mean - mean(ex_mean, na.rm=T),
...
esperance_vie_map <- 
  france_1800 %>% 
  left_join(ex_depts_comparison_depts %>% 
...
p_map <- 
  ...
  scale_x_continuous(limits = c(-6, 10)) +
  scale_y_continuous(limits = c(41, 51.5)) +
  scale_fill_gradient2("Écart à la moyenne, par département (années)", low = "red", mid = "white", high = "#71BC78", midpoint = 0, na.value = "grey",
...
# Sédentarité et migration intérieure en France au XIXe siècle
...
On agrège ces trois tableaux en un seul :
